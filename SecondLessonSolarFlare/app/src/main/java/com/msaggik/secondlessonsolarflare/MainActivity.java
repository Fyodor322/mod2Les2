...
public class MainActivity extends AppCompatActivity {
    ...
    // поля
    private final int[] delayMinute = {20, 42, 85}; // задержка прихода сигнала с Марса, Юпитера, Сатурна (в минутах)

    private final int[] volumeData = {14, 1250, 883}; // количество информации в час с Марса, Юпитера, Сатурна

    private final int[] lengthАFlashMinute = {60, 70, 85}; //длина вспышки на Марсе, Юпитера, Сатурна (в минутах)

    private final int[] volumeDataGeneral = {volumeData[0]/60 * lengthАFlashMinute[0], volumeData[1]/60 * lengthАFlashMinute[1], volumeData[2]/60 * lengthАFlashMinute[2]};

    private final int coreFrequency = 3; // скорость компьютера в секунду
    ...
    //метод конвертации времени вспышки из минуты в секунды
    private int lengthАFlashSecond(int lengthАFlashMinute) { return lengthАFlashMinute*60; }

    // МОДУЛЬ 2
    // метод определения времени обработки компьютером данных с одной планеты в секундах (скорость компьютера, общий размер данных)
    private int volumeTime(int coreFrequency, int volumeDataGeneral) {
        return  lengthАFlashSecond(volumeDataGeneral)/ coreFrequency;
    }

    // МОДУЛЬ 3 (использующий МОДУЛИ 1-2)
    // метод вычисления времени обработки компьютером приходящей информации в секундах (массив задержек, массив общих объёмов данных, скорость компьютера)
    private int coreTime(int[] delayMinute, int[] volumeDataGeneral, int frequency) {
        ...
        int[] volume = new int[volumeDataGeneral.length]; // создание пустого массива для времени обработки компьютером данных от спутника (в секундах)
        for (int i = 0; i < volume.length; i++) { // инициализация данного массива
            volume[i] = volumeTime(frequency, volumeDataGeneral[i]); // определение времени обработки компьютером данных от одного спутника
        }
